...
\title{\textbf{AI-Driven Epilepsy Genetic Variant Classification With ACMG
       Criteria and Retrieval-Augmented Generation}}

\author{
  M P Pushpalatha$^{1}$,\quad
  Srinidhi S G$^{1}$,\quad
  Bharath H$^{1}$\\[4pt]
  Sushant Kabbinada$^{1}$,\quad
  Aditya Patil$^{1}$
}
...
\date{
  ...
  JSS Science and Technology University, Mysuru, India\\[6pt]
  \textit{Running title:} AI epilepsy variant classifier with ACMG\\[4pt]
  \textit{Correspondence:} M P Pushpalatha;
  Mobile: +91 9141410120;
  Email: mppvin@jssstuniv.in
}
...
\medskip
\noindent\textbf{Keywords:} epilepsy; variant pathogenicity; ACMG/AMP;
retrieval-augmented generation; clinical genomics

% =============================================================================
\section{Introduction}

Epilepsy affects an estimated fifty million individuals worldwide
\citep{scheffer2017}. Clinical genomics has responded with expanded
...
The ACMG/AMP five-tier classification framework \citep{richards2015}
...
all findings under a layered rule system. \citet{nolan2022}
illustrate the scale of this challenge through a large-cohort
multigene panel study in which a substantial fraction of identified
variants carried uncertain or ambiguous clinical significance,
underscoring the need for systematic and reproducible classification
workflows.

Existing automation tools address only fragments of this problem.
InterVar \citep{intervar2017} translates the ACMG decision logic
...
\citep{autopvs1_2020} is narrowly scoped to a single criterion,
PVS1. Genome-wide deleteriousness scorers such as CADD
\citep{cadd2014} and REVEL \citep{revel2016} were not designed with
epilepsy-specific variant biology in mind. \citet{nicora2022}
treated ACMG evidence tallies as input features for a random forest,
...
widely cited despite their restricted scope. CADD \citep{cadd2014}
...
variant classes. REVEL \citep{revel2016} extended the ensemble
...
deep learning across genomics benchmarks \citep{parente2021}.
XGBoost \citep{chen2016xgboost} was therefore a natural fit---not
...
InterVar \citep{intervar2017} formalised the ACMG decision process
...
unavailable. AutoPVS1 \citep{autopvs1_2020} targeted the PVS1
criterion alone, using transcript-level annotations to judge
loss-of-function consequences. \citet{nicora2022} reframed the
problem: rather than encoding ACMG rules explicitly, they used ACMG
evidence tallies as input features for a random forest classifier.
...
The SHAP framework \citep{lundberg2017shap} decomposes each
...
\citep{lundberg2020trees} computes exact SHAP values for tree
ensembles in polynomial time, enabling attribution at every
inference call without a noticeable latency penalty.
\citet{watson2022} documented the growing expectation in clinical
genomics that predictions be accompanied by feature-level
explanations that practitioners can interrogate.

\subsection{Retrieval-Augmented Generation}

\citet{lewis2020rag} introduced retrieval-augmented generation
(RAG) as a method for grounding language model outputs in documents
fetched at query time, achieving markedly lower hallucination rates
than closed-book baselines on knowledge-intensive tasks. In
clinical NLP, BiomedRAG \citep{biomedrag2024} showed that pairing
a domain-specific retrieval index---built on PubMedBERT
\citep{pubmedbert2021} embeddings---with a biomedical corpus yields
...
\citep{clinvar2018} on the basis of well-established causal links
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{fig7_class_distribution_smote_genes.jpg}
...
\citep{pubmedbert2021} embeddings of epilepsy abstracts). ClinVar
...
\resizebox{\columnwidth}{!}{%
\begin{tikzpicture}
  \draw[thick,{-Stealth}](0,0)--(9.5,0)
        node[right,font=\normalsize]{$\hat{p}$};
  \foreach \x/\lbl in {0/0, 0.9/0.1, 2.7/0.3, 4.5/0.5, 6.3/0.7, 8.1/0.9, 9.0/1.0}
    \draw(\x,0.12)--(\x,-0.12) node[below,font=\normalsize]{\lbl};
  \fill[green!50,opacity=0.9](0,0.22)    rectangle(2.7,1.80);
  \fill[yellow!65,opacity=0.9](2.7,0.22) rectangle(6.3,1.80);
  \fill[red!40,opacity=0.9]  (6.3,0.22) rectangle(9.0,1.80);
  \node[font=\normalsize,text=green!60!black,align=center] at(1.35,1.01)
        {Confident\\Benign};
  \node[font=\normalsize,align=center] at(4.50,1.01)
        {Uncertain Zone\\4-vote adjudication};
  \node[font=\normalsize,text=red!70!black,align=center] at(7.65,1.01)
        {Confident\\Pathogenic};
  \draw[dashed,blue,thick](2.7,0.22)--(2.7,1.80);
  \draw[dashed,blue,thick](6.3,0.22)--(6.3,1.80);
  \node[font=\normalsize,text=blue,above] at(2.7,1.80) {0.3};
  \node[font=\normalsize,text=blue,above] at(6.3,1.80) {0.7};
\end{tikzpicture}%
}
...
XGBoost \citep{chen2016xgboost} was chosen on two grounds. First,
prior genomics benchmarks show gradient-boosted trees match or
exceed deep networks on structured tabular data \citep{parente2021}.
Second, XGBoost natively supports TreeSHAP \citep{lundberg2020trees},
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{fig2_feature_importance_fixed.jpg}
...
\subsection{ACMG/AMP Criteria Evaluation Logic}
\label{sec:acmg}

\setlength{\tabcolsep}{3.5pt}
\scriptsize
\begin{longtable}{p{1.1cm}p{3.5cm}p{3.5cm}p{2.2cm}p{0.8cm}p{2.2cm}}
...
\midrule
\endfirsthead
\multicolumn{6}{c}{\textit{Table~\thetable{} continued}} \\[2pt]
\toprule
\textbf{Gene} & \textbf{Associated Syndrome} & \textbf{Key Seizure Types} & \textbf{Age of Onset} & \textbf{Mech.} & \textbf{Severity} \\
\midrule
\endhead
...
Tavtigian~2018 Bayesian point model \citep{tavtigian2018}, which
...
training set was derived \citep{wolff2017}. Had we labelled SCN2A
...
current in disease-associated missense alleles \citep{zaman2018},
...
allele are typically unaffected \citep{dibbens2008}. We keep the
...
which give the same score regardless of which gene the variant is
in. We document in our source code that ACMG PP3 formally requires
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{fig10_per_gene_acmg_accuracy.jpg}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{fig3_ml_validation_test_full.jpg}
...
LOF/GOF gene dictionary (section~\ref{sec:acmg}) confines this
signal to genuinely haploinsufficient loci, preventing it from
inflating pathogenicity scores for the four GOF sodium-channel
genes in our panel, where the disease mechanism is channel
hyperactivation rather than protein loss.

\textbf{Why SHAP feedback instead of external predictors.}
PP3/BP4 in InterVar \citep{intervar2017} relies on SIFT and
...
attribution scores based on gene identity. We note that ACMG PP3
formally calls for evidence from multiple independent computational
tools; our single-source approximation trades breadth for
...
This work introduced a ten-layer epilepsy variant classification
...
The authors are grateful to JSS Science and Technology University,
Mysuru, for providing the institutional environment that made this
work possible. Variant data were drawn from ClinVar (NCBI, NIH)
and the Genome Aggregation Database (gnomAD, Broad Institute),
both of which are invaluable open resources for the genomics
community. Literature embeddings were generated using PubMedBERT
models available through the Hugging Face model hub, and language
model inference was carried out via the Groq API using Qwen3-32B.
...
\bibitem[Dibbens et al.(2008)]{dibbens2008}
Dibbens LM, Tarpey P, Hynes K, Bayly MA, Scheffer IE, Smith R,
Bomar J, Sutton E, Vandeleur L, Shoubridge C \textit{et al} 2008
X-linked protocadherin 19 mutations cause female-limited epilepsy
and cognitive impairment. \textit{Nat. Genet.} \textbf{40} 776--781

\bibitem[{Epi25 Collaborative}(2019)]{epi25_2019}
Epi25 Collaborative 2019 Ultra-rare genetic variation in the
epilepsies: A whole-exome sequencing study of 17,606 individuals.
\textit{Am. J. Hum. Genet.} \textbf{105} 267--282

\bibitem[Gu et al.(2021)]{pubmedbert2021}
Gu Y, Tinn R, Cheng H, Lucas M, Usuyama N, Liu X, Naumann T,
Gao J and Poon H 2021 Domain-specific language model pretraining
for biomedical natural language processing.
\textit{ACM Trans. Comput. Healthcare} \textbf{3} 1--23

\bibitem[Ioannidis et al.(2016)]{revel2016}
Ioannidis NM, Rothstein JH, Pejaver V, Middha S, McDonnell SK,
Baheti S, Musolf A, Li Q, Holzinger E, Karyadi D \textit{et al}
2016 REVEL: An ensemble method for predicting the pathogenicity of
rare missense variants. \textit{Am. J. Hum. Genet.} \textbf{99} 877--885

\bibitem[Karczewski et al.(2020)]{karczewski2020}
Karczewski KJ, Francioli LC, Tiao G, Cummings BB, Alf\"{o}ldi J,
Wang Q, Collins RL, Laricchia KM, Ganna A, Birnbaum DP
\textit{et al} 2020 The mutational constraint spectrum quantified
from variation in 141,456 humans. \textit{Nature} \textbf{581}
434--443

\bibitem[Kircher et al.(2014)]{cadd2014}
Kircher M, Witten DM, Jain P, O'Roak BJ, Cooper GM and
Shendure J 2014 A general framework for estimating the relative
pathogenicity of human genetic variants.
\textit{Nat. Genet.} \textbf{46} 310--315

\bibitem[Landrum et al.(2018)]{clinvar2018}
Landrum MJ, Lee JM, Benson M, Brown GR, Chao C, Chitipiralla S,
Gu B, Hart J, Hoffman D, Jang W \textit{et al} 2018 ClinVar:
improving access to variant interpretations and supporting evidence.
\textit{Nucleic Acids Res.} \textbf{46} D1062--D1067

\bibitem[Lewis et al.(2020)]{lewis2020rag}
Lewis P, Perez E, Piktus A, Petroni F, Karpukhin V, Oguz Y,
Sch\"{u}tze H, \c{C}elebi-T\"{u}rk N, Gazit N, Stoyanov V
\textit{et al} 2020 Retrieval-augmented generation for
knowledge-intensive NLP tasks; in \textit{Advances in Neural
Information Processing Systems 33} (Curran Associates: Red Hook, NY)
pp 9459--9474

\bibitem[Li and Wang(2017)]{intervar2017}
Li Q and Wang K 2017 InterVar: Clinical interpretation of genetic
variants by the 2015 ACMG-AMP guidelines.
\textit{Am. J. Hum. Genet.} \textbf{100} 267--280

\bibitem[Lundberg and Lee(2017)]{lundberg2017shap}
Lundberg SM and Lee S-I 2017 A unified approach to interpreting
model predictions; in \textit{Advances in Neural Information
Processing Systems 30} (Curran Associates: Red Hook, NY) pp
4765--4774
...
\textit{Nat. Mach. Intell.} \textbf{2} 56--67

\bibitem[Nicora et al.(2022)]{nicora2022}
Nicora G, Limongelli I, Gambardella A, Striano P and Bellazzi R
2022 A machine learning approach based on ACMG/AMP guidelines for
genomic variant classification and prioritization.
\textit{Sci. Rep.} \textbf{12} 2542

\bibitem[Nolan et al.(2022)]{nolan2022}
Nolan D, Filer A, Curry M, Kelso M, Wu Y, Guo R, Crepeau K,
Aggarwal V and Lapin B 2022 Multigene panel testing in a large
cohort of adults with epilepsy.
\textit{Neurol. Genet.} \textbf{8} e650

\bibitem[Parente et al.(2021)]{parente2021}
Parente DJ, Stark Z, Long JM, Hickey SE, Mori M and Brokamp E 2021
PolyBoost: An enhanced genomic variant classifier using extreme
gradient boosting.
\textit{Proteomics Clin. Appl.} \textbf{15} e1900124

\bibitem[Richards et al.(2015)]{richards2015}
Richards S, Aziz N, Bale S, Bick D, Das S, Gastier-Foster J,
Grody WW, Hegde M, Lyon E, Spector E \textit{et al} 2015 Standards
and guidelines for the interpretation of sequence variants.
\textit{Genet. Med.} \textbf{17} 405--424

\bibitem[Scheffer et al.(2017)]{scheffer2017}
Scheffer IE, Berkovic S, Capovilla G, Connolly MB, French J,
Guilhoto L, Hirsch E, Jain S, Mathern GW, Mosh\'{e} SL
\textit{et al} 2017 ILAE classification of the epilepsies: Position
paper of the ILAE Commission for Classification and Terminology.
\textit{Epilepsia} \textbf{58} 512--521

\bibitem[Tavtigian et al.(2018)]{tavtigian2018}
Tavtigian SV, Greenblatt MS, Harrison SM, Nussbaum RL, Prabhu SA,
Boucher KM, Biesecker LG and ClinGen Sequence Variant Interpretation
Working Group 2018 Modeling the ACMG/AMP variant classification
guidelines as a Bayesian classification framework.
\textit{Genet. Med.} \textbf{20} 1054--1060

\bibitem[Watson(2022)]{watson2022}
Watson DS 2022 Interpretable machine learning for genomics.
\textit{Hum. Genet.} \textbf{141} 1499--1513

\bibitem[Wolff et al.(2017)]{wolff2017}
Wolff M, Johannesen KM, Hedrich UB, Masnada S, Rubboli G,
Gardella E, Lesca G, Ville D, Milh M, Villard L \textit{et al} 2017
Genetic and phenotypic heterogeneity suggest therapeutic implications
in SCN2A-related disorders.
\textit{Brain} \textbf{140} 1316--1336

\bibitem[Xiang et al.(2020)]{autopvs1_2020}
Xiang J, Li X, Li M, Fang Q, Zhao L, Pan Q, Shen L, Tang J,
Luo X and Hu Z 2020 AutoPVS1: An automatic classification tool for
PVS1 interpretation of null variants.
\textit{Hum. Mutat.} \textbf{41} 1488--1498

\bibitem[Xiong et al.(2025)]{biomedrag2024}
Xiong H, Wang Q, Jiang L, Chen Q, Yao R, Liu X, Xu L and Jiang X
2025 BiomedRAG: A retrieval augmented large language model for
biomedicine.
\textit{J. Biomed. Inform.} \textbf{162} 104769

\bibitem[Zaman et al.(2018)]{zaman2018}
Zaman T, Helbig KL, Clatot J, Thompson CH, Kang SK, Strulovich R,
Goldberg-Stern H, Moyes C, Dlugos DJ, Helbig I \textit{et al} 2018
Mutations in SCN3A cause early infantile epileptic encephalopathy.
\textit{Ann. Neurol.} \textbf{83} 703--717
