% ============================================================
%  IJMR-formatted version of the epilepsy variant classification paper
%  Journal: Indian Journal of Medical Research (IJMR)
%  Guidelines: https://ijmr.org.in/for-authors/
%  Content identical to epilepsy_variant_classification.tex
% ============================================================
...
% ── TikZ styles (unchanged from original) ────────────────────────────────────
\tikzset{
...
\fancyhead[L]{\small\textit{M P Pushpalatha et al}}
\fancyhead[R]{\small\textit{Indian J Med Res}}
\fancyfoot[C]{\thepage}
...
% ── Title ────────────────────────────────────────────────────────────────────
\begin{center}
{\LARGE\bfseries AI-Driven Epilepsy Genetic Variant Classification With
ACMG Criteria and Retrieval-Augmented Generation}

\vspace{0.4cm}
{\small\textit{Running title: AI epilepsy variant classifier with ACMG \& RAG}}

\vspace{0.6cm}

% ── Authors (IJMR format: Full Name, Highest Degree, Designation) ────────────
{\normalsize
Dr. M P Pushpalatha\textsuperscript{1},
Srinidhi S G\textsuperscript{2},
Bharath H\textsuperscript{2},
Sushant Kabbinada\textsuperscript{2}
\& Aditya Patil\textsuperscript{2}
}

\vspace{0.3cm}
{\small
\textsuperscript{1}Professor, Department of Computer Science \& Engineering,\\
JSS Science and Technology University, Mysuru 570\,006, Karnataka, India\\
\textit{e-mail: mppvin@jssstuniv.in}

\vspace{0.2cm}
\textsuperscript{2}Department of Computer Science \& Engineering,\\
JSS Science and Technology University, Mysuru 570\,006, Karnataka, India\\
\textit{e-mail: srinidhisg1@gmail.com, bharath62004@gmail.com,
sushant.k2150@gmail.com, adityapatil0275@gmail.com}
}
...
% ── Structured Abstract (IJMR: Background \& objectives / Methods / Results /
%    Interpretation \& conclusions; max 250 words) ────────────────────────────
\noindent\rule{\textwidth}{0.4pt}
\begin{description}[leftmargin=0pt, labelwidth=!, labelsep=0.3em,
                    font=\normalfont\bfseries, itemsep=2pt]

\item[Background \& objectives:]
Epilepsy affects an estimated fifty million individuals worldwide, yet
manual classification of genetic variants against the ACMG/AMP guidelines
remains labour-intensive and inconsistent across laboratories.
No existing tool jointly derives its computational-evidence criterion
from its own model output, queries live genomic databases at inference time,
and synthesises a human-readable clinical report via a language model.
This study presents a ten-layer automated pipeline addressing these gaps.

\item[Methods:]
A 93-dimensional feature vector was engineered from seven clinician-entered
fields (gene, chromosome, alleles, consequence, variant type, and review
status) and used to train an isotonic-calibrated XGBoost classifier on
52,323 germline ClinVar variants across 26 epilepsy genes.
TreeSHAP attributions from the classifier were recycled to satisfy the
ACMG PP3/BP4 computational-evidence criterion, eliminating dependence on
SIFT or PolyPhen-2. A 14-criterion ACMG engine implementing the Tavtigian
Bayesian point model was validated against 200 expert-panel ClinVar labels.
For pathogenic or uncertain predictions ($\hat{p}\in[0.3,0.7]$), four
evidence fetchers (ClinVar, gnomAD, PubMed via FAISS, PharmGKB) executed in
parallel; a four-voter adjudicator resolved ambiguity; a contradiction
detector flagged inter-source conflicts; and Qwen3-32B synthesised a
structured clinical narrative.

\item[Results:]
On 7,673 held-out variants the classifier attained 89.89\% accuracy,
AUC 0.9446, F1 0.8538, and Brier score 0.0761.
The maximum cross-fold metric gap was 0.007.
The ACMG engine agreed with expert-panel labels at 98.5\% (197/200).
Total wall-clock time remained within ten seconds without requiring any
phenotype input.

\item[Interpretation \& conclusions:]
The pipeline demonstrates that recycling a classifier's own SHAP
attributions for ACMG PP3/BP4 is a viable, self-contained alternative to
external pathogenicity scorers, and that conditional retrieval-augmented
generation can deliver evidence-grounded clinical narratives within
practical latency bounds. This system is intended for research use only
...
% ── Keywords (alphabetical, MeSH-preferred) ──────────────────────────────────
\noindent\textbf{Key words}
ACMG/AMP guidelines $\bullet$
clinical genomics $\bullet$
epilepsy $\bullet$
large language model $\bullet$
retrieval-augmented generation $\bullet$
variant pathogenicity $\bullet$
XGBoost
...
Scheffer IE, Berkovic S, Capovilla G, Connolly MB, French J, Guilhoto L,
et al. ILAE classification of the epilepsies: Position paper of the ILAE
Commission for Classification and Terminology.
\textit{Epilepsia} 2017; 58: 512--21.

\bibitem{epi25_2019}
Epi25 Collaborative. Ultra-rare genetic variation in the epilepsies:
A whole-exome sequencing study of 17,606 individuals.
\textit{Am J Hum Genet} 2019; 105: 267--82.
...
\textit{Genet Med} 2015; 17: 405--24.

\bibitem{nolan2022}
Nolan D, Fink JK, de Lonlay P. Multigene panel testing in a large cohort
of adults with epilepsy.
\textit{Neurol Genet} 2022; 8: e650.

\bibitem{intervar2017}
Li Q, Wang K. InterVar: Clinical interpretation of genetic variants by
the 2015 ACMG-AMP guidelines.
\textit{Am J Hum Genet} 2017; 100: 267--80.

\bibitem{autopvs1_2020}
Xiang J, Li Q, Li W, Wang Z, Mei H, Jiang T, et al.
AutoPVS1: An automatic classification tool for PVS1 interpretation of
null variants.
\textit{Hum Mutat} 2020; 41: 1488--98.
...
genetic variants.
\textit{Nat Genet} 2014; 46: 310--5.

\bibitem{revel2016}
Ioannidis NM, Rothstein JH, Pejaver V, Middha S, McDonnell SK,
Baheti S, et al. REVEL: An ensemble method for predicting the
pathogenicity of rare missense variants.
\textit{Am J Hum Genet} 2016; 99: 877--85.

\bibitem{nicora2022}
Nicora G, Limongelli I, Gambelli P, Memeo R, Magalini S, Petrella S,
et al. A machine learning approach based on ACMG/AMP guidelines for
genomic variant classification and prioritization.
\textit{Sci Rep} 2022; 12: 2542.

\bibitem{chen2016xgboost}
Chen T, Guestrin C. XGBoost: A scalable tree boosting system. In:
\textit{Proc 22nd ACM SIGKDD Int Conf KDD}; 2016. p. 785--94.

\bibitem{parente2021}
Parente DJ, Bhatt DL, Bhatt M. PolyBoost: An enhanced genomic variant
classifier using extreme gradient boosting.
\textit{PROTEOMICS Clin Appl} 2021; 15: e1900124.

\bibitem{tavtigian2018}
Tavtigian SV, Greenblatt MS, Harrison SM, Nussbaum RL, Prabhu SA,
Boucher KM, et al. Modeling the ACMG/AMP variant classification
guidelines as a Bayesian classification framework.
\textit{Genet Med} 2018; 20: 1054--60.
...
evidence.
\textit{Nucleic Acids Res} 2018; 46: D1062--7.
...
141,456 humans.
\textit{Nature} 2020; 581: 434--43.

\bibitem{lundberg2017shap}
Lundberg SM, Lee SI. A unified approach to interpreting model predictions.
In: \textit{Adv Neural Inf Process Syst 30}; 2017. p. 4765--74.
...
trees.
\textit{Nat Mach Intell} 2020; 2: 56--67.

\bibitem{watson2022}
Watson DS. Interpretable machine learning for genomics.
\textit{Hum Genet} 2022; 141: 1499--513.
...
\textit{Adv Neural Inf Process Syst 33}; 2020. p. 9459--74.

\bibitem{biomedrag2024}
Xiong H, Jin Q, Lu Z, Zhang A. BiomedRAG: A retrieval augmented large
language model for biomedicine.
\textit{J Biomed Inform} 2025; 162: 104769.
...
language processing.
\textit{ACM Trans Comput Healthcare} 2021; 3: 1--23.

\bibitem{wolff2017}
Wolff M, Johannesen KM, Hedrich UBS, Masnada S, Rubboli G,
Gardella E, et al. Genetic and phenotypic heterogeneity suggest
therapeutic implications in SCN2A-related disorders.
\textit{Brain} 2017; 140: 1316--36.

\bibitem{zaman2018}
Zaman T, Helbig I, Bali\={c} S, Thomas RH, Mei D, Santoro D, et al.
Mutations in SCN3A cause early infantile epileptic encephalopathy.
\textit{Ann Neurol} 2018; 83: 703--17.
...
\textit{Nat Genet} 2008; 40: 776--81.
